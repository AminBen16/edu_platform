...
class _AssignmentDetailsScreenState extends State<AssignmentDetailsScreen> {
  ...
  Future<void> _pickFile() async {
    // Use image_picker which is already available
    try {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text(widget.assignment['title'] ?? 'Assignment', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text('Subject: ${widget.assignment['subject'] ?? 'N/A'}', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 4),
            Text('Teacher: ${widget.assignment['teacher'] ?? 'N/A'}', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 4),
            Text('Due Date: ${widget.assignment['dueDate'] ?? 'N/A'}', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 16),
            const Divider(),
            const SizedBox(height: 16),
            Text('Description', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 8),
            Text(widget.assignment['description'] ?? 'No description', style: Theme.of(context).textTheme.bodyMedium),
    ...
    if (status == 'graded' || status == 'submitted') {
      return Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text('Status: ${_getStatusText(status)}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: _getStatusColor(status))),
          if (widget.assignment['grade'] != null) ...[
            const SizedBox(height: 8),
            Text('Grade: ${widget.assignment['grade']}/${widget.assignment['totalPoints'] ?? 100}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.green)),
          ],
          if (widget.assignment['feedback'] != null) ...[
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 4),
                Text(widget.assignment['feedback'].toString(), style: TextStyle(fontSize: 14, color: Colors.blue[700])),
              ]),
            ),
          ],
        ],
      ...
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Submit Your Work', style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 16),
          ...
          TextField(
            ...
            decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'Add a comment (optional)'),
            maxLines: 3,
          ...
          Center(
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: Colors.green, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 48, vertical: 12)),
              child: _isSubmitting
