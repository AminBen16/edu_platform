...
router.get('/lesson/:id', auth_1.protect, async (req, res) => {
    const { id } = req.params;
    const user = req.user;
    try {
        ...
        if (user.role === database_2.Role.STUDENT) {
        ...
        else if (user.role === database_2.Role.TEACHER && lesson.teacherId !== (await database_1.prisma.teacher.findUnique({ where: { userId: user.id } }))?.id) {
...
router.get('/quiz/:id', auth_1.protect, async (req, res) => {
    const { id } = req.params;
    const user = req.user;
    try {
        ...
        // Ensure only enrolled students, teachers, or admins can download
        if (user.role === database_2.Role.STUDENT) {
            const hasAttempted = await database_1.prisma.quizAttempt.count({
        ...
        else if (user.role === database_2.Role.TEACHER && quiz.teacherId !== (await database_1.prisma.teacher.findUnique({ where: { userId: user.id } }))?.id) {
...
router.get('/assignment/:id', auth_1.protect, async (req, res) => {
    const { id } = req.params;
    const user = req.user;
    try {
        ...
        // Ensure only enrolled students, teachers, or admins can download
        if (user.role === database_2.Role.STUDENT) {
            const isEnrolledInLesson = await database_1.prisma.enrollment.findFirst({
        ...
        else if (user.role === database_2.Role.TEACHER && assignment.teacherId !== (await database_1.prisma.teacher.findUnique({ where: { userId: user.id } }))?.id) {
...
router.get('/submissions/:assignmentId', auth_1.protect, (0, auth_1.authorize)(database_2.Role.TEACHER, database_2.Role.ADMIN, database_2.Role.SUPER_ADMIN), async (req, res) => {
    ...
    try {
        ...
        if (user.role === database_2.Role.TEACHER && assignment.teacherId !== (await database_1.prisma.teacher.findUnique({ where: { userId: user.id } }))?.id) {
...
async function generateLessonPDF(lesson) {
...
async function generateQuizPDF(quiz) {
    return new Promise((resolve, reject) => {
        const doc = new pdfkit_1.default({ margin: 50 });
        const buffers = [];
        doc.on('data', buffers.push.bind(buffers));
