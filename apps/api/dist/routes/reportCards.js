...
router.get('/', auth_1.protect, async (req, res) => {
    try {
        ...
        const reportCards = await database_1.prisma.reportCard.findMany({
            where,
            include: {
                student: {
...
router.get('/:id', auth_1.protect, async (req, res) => {
    const { id } = req.params;
    try {
        const reportCard = await database_1.prisma.reportCard.findFirst({
...
router.post('/', auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        const existing = await database_1.prisma.reportCard.findFirst({
        ...
        const assessmentResults = await database_1.prisma.assessmentResult.findMany({
            where: {
                studentId,
                assessment: {
                    termId,
        ...
        const calculatedOverallGrade = scores.length > 0
            ...
            : overallGrade || 0;
        const reportCard = await database_1.prisma.reportCard.create({
            data: {
                studentId,
                termId,
                classId,
...
router.put('/:id', auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        const updated = await database_1.prisma.reportCard.update({
            where: { id },
            data: {
                overallGrade,
                attendancePercent,
        ...
        if (subjects && Array.isArray(subjects)) {
            for (const subjectUpdate of subjects) {
                if (subjectUpdate.id) {
                    await database_1.prisma.reportCardSubject.update({
...
router.post('/:id/publish', auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        const published = await database_1.prisma.reportCard.update({
            where: { id },
            data: {
                isPublished: true,
