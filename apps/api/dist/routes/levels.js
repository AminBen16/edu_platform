...
router.get('/', auth_1.protect, async (req, res) => {
    try {
        ...
        if (curriculumId) {
            ...
            const curriculum = await database_1.prisma.curriculum.findFirst({
        ...
        const levels = await database_1.prisma.curriculumLevel.findMany({
            where,
            include: {
...
router.post('/initialize-uganda', auth_1.protect, async (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Only admins can initialize curriculum' });
    }
    try {
        // Check if curriculum exists, create if not
        let curriculum = await database_1.prisma.curriculum.findFirst({
            where: { schoolId: req.user.schoolId }
        });
        if (!curriculum) {
            curriculum = await database_1.prisma.curriculum.create({
                data: {
        ...
        const existingLevels = await database_1.prisma.curriculumLevel.findMany({
            where: { curriculumId: curriculum.id }
        });
        ...
        const createdLevels = await Promise.all(DEFAULT_UGANDA_LEVELS.map(levelData => database_1.prisma.curriculumLevel.create({
            data: {
                code: levelData.code,
                name: levelData.name,
...
router.post('/', auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        let curriculum = curriculumId
            ? await database_1.prisma.curriculum.findUnique({ where: { id: curriculumId } })
            : await database_1.prisma.curriculum.findFirst({ where: { schoolId: req.user.schoolId } });
        if (!curriculum) {
            curriculum = await database_1.prisma.curriculum.create({
                data: {
                    name: 'Uganda Competency Based Curriculum',
        ...
        const newLevel = await database_1.prisma.curriculumLevel.create({
            data: {
                code,
                name,
                level: level || 0,
...
router.put('/:id', auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        const existingLevel = await database_1.prisma.curriculumLevel.findUnique({
            where: { id },
        ...
        const updatedLevel = await database_1.prisma.curriculumLevel.update({
            where: { id },
...
router.delete('/:id', auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        await database_1.prisma.curriculumLevel.delete({ where: { id } });
        res.status(204).send();
    }
    catch (error) {
...
router.get('/:id/subjects', auth_1.protect, async (req, res) => {
    const { id } = req.params;
    try {
...
router.post('/:id/subjects', auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        const existingMapping = await database_1.prisma.levelSubject.findFirst({
            where: { levelId: id, subjectId }
        });
        if (existingMapping) {
            return res.status(400).json({ error: 'Subject already added to this level' });
        }
        const levelSubject = await database_1.prisma.levelSubject.create({
            data: {
                levelId: id,
                subjectId,
                isEnabled
...
router.delete('/:id/subjects/:subjectId', auth_1.protect, async (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Only admins can remove subjects' });
    }
    const { id, subjectId } = req.params;
    try {
        await database_1.prisma.levelSubject.deleteMany({
            where: { levelId: id, subjectId }
        });
        res.status(204).send();
    }
