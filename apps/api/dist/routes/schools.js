...
router.get('/', auth_1.protect, async (req, res) => {
    try {
        ...
        const schools = await database_1.prisma.school.findMany({
...
router.put('/:id', auth_1.protect, async (req, res) => {
    ...
    try {
        const school = await database_1.prisma.school.findUnique({ where: { id } });
        if (!school) {
            return res.status(404).json({ error: 'School not found' });
        }
        ...
        const updatedSchool = await database_1.prisma.school.update({
            where: { id },
            data: {
                name: name || undefined,
...
router.delete('/:id', auth_1.protect, async (req, res) => {
    ...
    const { id } = req.params;
    try {
        await database_1.prisma.school.delete({ where: { id } });
        res.status(204).send();
    }
    catch (error) {
