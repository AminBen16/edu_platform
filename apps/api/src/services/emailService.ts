...
class EmailService {
  ...
  static async sendInvitationEmail(email: string, name: string, invitationCode: string, schoolName: string): Promise<boolean> {
    const html = `
  ...
  static async sendDeletionConfirmationEmail(email: string, name: string, deletionToken: string, schoolName: string): Promise<boolean> {
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8f9fa;">
