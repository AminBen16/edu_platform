...
const peerConnections: Map<string, Set<string>> = new Map();
const roomParticipants: Map<string, Map<string, { userId: string; userName: string; role: string }>> = new Map();

// All routes require authentication
router.use(protect);

// POST /webrtc/room/:roomCode/join - Join a WebRTC room
router.post('/room/:roomCode/join', async (req: RequestWithUser, res: Response) => {
...
router.post('/room/:roomCode/leave', async (req: RequestWithUser, res: Response) => {
  const { roomCode } = req.params;
  ...
  try {
    ...
    const session = await prisma.liveSession.findUnique({ where: { roomCode } });
    if (session) {
...
router.get('/room/:roomCode/participants', async (req: RequestWithUser, res: Response) => {
  const { roomCode } = req.params;
...
router.post('/signal', async (req: RequestWithUser, res: Response) => {
  ...
  try {
    ...
    const signals = globalThis as any;
...
router.get('/signal/:roomCode/:peerId', async (req: RequestWithUser, res: Response) => {
  const { roomCode, peerId } = req.params;
  const { id: userId } = req.user!;

  try {
    const signals = (globalThis as any)._webrtcSignals;
    if (!signals) {
...
router.post('/room/:roomCode/ping', async (req: RequestWithUser, res: Response) => {
  const { roomCode } = req.params;
  const { id: userId } = req.user!;
