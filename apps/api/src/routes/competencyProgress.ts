...
router.get('/', protect, async (req, res) => {
  try {
    ...
    if (competencyId) {
      where.competencyId = competencyId as string;
    }
      ...
      where.student = {
        enrollments: {
          some: {
            ...
            ...(subjectId ? { class: { subjectId: subjectId as string } } : {})
          }
        }
      };
    }
...
router.get('/student/:studentId', protect, async (req, res) => {
  ...
  try {
    ...
    if (subjectId) {
      where.competency = {
        topic: { subjectId: subjectId as string }
      };
    }
...
router.get('/competency/:competencyId', protect, async (req, res) => {
  ...
  try {
    ...
    type MasteryKey = keyof typeof distribution;
    progress.forEach((p: { masteryLevel: string }) => {
      if (p.masteryLevel in distribution) {
        distribution[p.masteryLevel as MasteryKey]++;
      }
    });
...
router.post('/', protect, async (req, res) => {
  ...
  try {
    ...
    const existing = await prisma.competencyProgress.findFirst({
      where: { studentId, competencyId }
    });

    if (existing) {
      // Update existing
      const updated = await prisma.competencyProgress.update({
    ...
    const progress = await prisma.competencyProgress.create({
      data: {
        studentId,
        competencyId,
        masteryLevel,
...
router.get('/mastery-levels', protect, async (req, res) => {
